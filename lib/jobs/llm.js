...
export async function parseResume(resumeText) {
  const prompt = `You are a resume parser. Extract structured data
from the resume text below. Return ONLY valid JSON.
...
Return exactly this JSON structure:
{
  ...
  return await callLLM(prompt, "object");
}

export async function batchAnalyzeJobs(resumeProfile, jobs) {
  const truncatedJobs = jobs.map((job, index) => ({
    index,
    jobTitle: job.jobTitle,
    companyName: job.companyName,
    description: job.jobDescription?.slice(0, 220) || "",
  }));

  const prompt = `You are a job matching expert. Analyze all jobs
against the resume profile. Return ONLY a valid JSON array.
No markdown. No explanation. No code blocks.
...
For each job return:
{
  ...
  "aiSummary": "1 sentence role summary",
  "whyItMatches": "1-2 sentences why this fits the user",
  "resumeImprovements": ["suggestion 1", "suggestion 2", "suggestion 3"]
}
