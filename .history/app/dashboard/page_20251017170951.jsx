...
export default function Dashboard() {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Failed to fetch credits:", errData);
      throw new Error(errData.error || "Failed to fetch credits");
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-grow">
        <section className="text-center px-4 sm:px-6 pt-12 pb-8">
          ...
          {credits !== null && (
            <p className="mt-4 text-green-600 font-medium">Your Credits: {credits}</p>
          )}
        </section>
            ...
            {allTemplates.map((template, index) => (
              <div key={index} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 border border-gray-200">
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-600 mb-4">{template.description}</p>
                  <Link href={`/resume-form?template=${template.slug}`} className="block w-full text-center py-3 bg-yellow-400 text-black rounded-lg hover:bg-green-600 transition font-medium">
                    Use This Template
                  </Link>
                </div>
              </div>
