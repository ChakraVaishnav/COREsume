...
export async function GET(req) {
  try {
    ...
    try {
      ...
      // Most likely the `template` field doesn't exist on the Rating model.
      console.debug('prisma findFirst by template failed, falling back to userId-only search', prismaErr?.message || prismaErr);
      try {
        existing = await prisma.rating.findFirst({ where: { userId } });
      } catch (err2) {
        console.error('prisma findFirst by userId also failed', err2);
        throw err2;
      }
    }
