...
function ResumeForm() {
  ...
  const handleAISuggestion = async (type, payload) => {
    ...
    if (dontRemind) {
      if(!form.jobrole || !form.experience){
        setShowToastRequired[true];
        setPendingAISuggestion(null);
        return;
      }
      const res=await runAISuggestion(pendingAISuggestion.type, pendingAISuggestion.payload);
      if(!res){
        return res;
      }
      await deductCredit();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col">
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        {showToastRequired && (
          <div className="fixed top-4 right-4 bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-3 rounded-xl shadow-lg border border-red-400 z-50">
            <p className="font-semibold">Job role and </p>
            <p className="text-sm">exp level are needed</p>
          </div>
        )}
